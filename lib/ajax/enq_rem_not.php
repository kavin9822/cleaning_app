...
$enquiry_remainder_tab = $tablePrefix . 'enquiry_remainder';  

$entityID = $ajxSess->get('user')['entity_ID'];

$query = "SELECT COUNT(*) AS MailCount FROM $enquiry_remainder_tab a JOIN $user_tab s ON s.ID = a.sendfromUser_ID JOIN $user_tab f ON f.ID = a.sendtoUser_ID where a.ApprovalStatus=0";
