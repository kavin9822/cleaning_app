    ...
    <div class="box-header">
        ...
        <form class="form-horizontal" action="<?php echo $home . '/' . $module . '/' . $controller . '/' . $method; ?>" method="post">
            ...
            <div class="row hidden-print">
                ...
                <div class="col-md-8 col-xs-2"><?php if(isset($widget_2)){echo $widget_2;} ?></div>
                <div class="col-md-2 col-xs-10 text-right">
                 ...
                 <!--   <button class="btn btn-info margin" onclick="javascript:window.print()">Print</button>
                    <?php if ($wp): ?>
                        ...
                        <?php if(isset($popdf) || isset($dcpdf)){?>
                       
                        <!-- <input class="btn btn-info margin" style="background-color:red; border:red;" type="submit" name="req_from_list_view" value="pdf" onclick="">  -->

                        <a href="#"  class="btn btn-info margin" id="PDF" style="background-color:red; border:red;" type="submit" name="req_from_list_view" disabled  >Generate PDF </a>
                      
                        <?php }?>

                    <!-- <input class="btn btn-info margin" type="submit" name= "req_from_list_view" value="View" onclick="return restrictCheck('view!');">  -->
                    <a href="<?php echo $home;?>/resource/report/<?php echo $cur_time ?>_Report.xls" download><input type="button" class="btn btn-success margin" value="Download Report"></a>
                </div>
            </div>
                                    ...
                                    <?php endforeach; ?>     
                                      ...
                                      <tr class="odd hidden-print">   	
                                        <th style="width:30px;" class="sorting_disabled" colspan="2">
                                            <input tabindex="1" class="btn btn-info fa" type="submit" name="list" value="Clear" onclick="<?php $FmData='';?>" /> 
                                        </th>
                                            <?php 
                                            if(!$ID_column_required){
                             ...
                             ?>
                                <tbody>
                                    ...
                                    <tr class="table-striped" role="row" >
                                            ...
                                            <td class="hidden-print"> <input style="width: 20px;" type="checkbox" name ="ycs_ID" id="<?php if(isset($popdf)|| isset($dcpdf)){ echo "checkbox_".$sql_data_value[0] ; } else {echo "checkbox";} ?>" onclick=" <?php if(isset($popdf)){ echo 'downloadpdf(this.value,\'PDF\')';} if(isset($dcpdf)){ echo 'dcdownloadpdf(this.value,\'PDF\')'; } ?> " value="<?php echo $sql_data_value[0]; ?>"></td>
...
function paginationHTML($page, $lastPage)
{
  ...
  $result ='<div class="col-md-2 form-group"> 
       <div class="input-group input-group-sm">
                <input type="number" class="form-control" name="pagenogo" id="pagenogo" value="" min="1">
